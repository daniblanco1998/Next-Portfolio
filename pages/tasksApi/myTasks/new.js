...
export default function TaskFormPage(){
  ...
  const createTask = async () => {
    try {
      await fetch('https://next-portfolio-fvv3svzna-daniblanco1998.vercel.app/api/tasks', {
        method: 'POST',
        headers: {
  ...
  const upDateTask = async () => {
    try {
      await fetch(`https://next-portfolio-fvv3svzna-daniblanco1998.vercel.app/api/tasks/${query.id}`, {
        method: 'PUT',
        headers: {
          "Content-Type":"application/json"
        },
  ...
  const getTask = async() => {
    const res = await fetch(`https://next-portfolio-fvv3svzna-daniblanco1998.vercel.app/api/tasks/${query.id}`)
